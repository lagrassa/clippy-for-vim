...
cpdef shapes.Shape primPrimCut(shapes.Prim p1, shapes.Prim p2, bool isect = False):
    ...
    if not geom.bboxOverlap(p1.bbox(), p2.bbox()):
        # no overlap
        return shapes.Shape([], util.Ident) if isect else shapes.Shape([p1], p1.thingOrigin)
    ...
    np.dot(planes2, verts1, fxv)        # reuse fxv
    if np.any(np.all(fxv >= -tiny, axis=1)):
        # some plane of p2 separates
        return shapes.Shape([], util.Ident) if isect else shapes.Shape([p1], p1.thingOrigin) 
    if np.all(np.all(fxv < tiny, axis=0)):
        # p1 is completely inside p2 (all verts inside)
        return shapes.Shape([p1], p1.thingOrigin) if isect else shapes.Shape([], util.Ident)
    return shapes.Shape(primPrimCutAux(p1, p2, isect), p1.thingOrigin)
...
cdef tuple facePrimCut(np.ndarray[np.float64_t, ndim=1] plane,
    ...
    if np.any(inv) and not np.any(outv): # everything inside
        return (None, geom.convexHullPrim(verts[:, inv], prim.thingOrigin))
    elif np.any(outv) and not np.any(inv): # everything outside
        return (geom.convexHullPrim(verts[:, outv], prim.thingOrigin), None)
    ...
    return (geom.convexHullPrim(np.hstack([verts[:, outv], cutV]), prim.thingOrigin),
            geom.convexHullPrim(np.hstack([verts[:, inv], cutV]), prim.thingOrigin))
